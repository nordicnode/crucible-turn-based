...
use crucible_sim::{
    fog::FogView,
    map::{MAP_SIZE, MAP_TILES},
    unit_stats, BuildingType, Game, Player, ResourceBundle, UnitType, FOG_MEMORY_TURNS,
    HQ_INCOME_PER_TURN, MATCH_TIMEOUT_TURNS, REFINERY_BASE_YIELD_PER_TURN,
};
...
pub const SINGLE_FEATURE_DIM: usize = 120;
...
pub struct FeatureInput {
    /// Legacy activation number (starts at 1).
    pub turn: i32,
    /// Player-facing round number.
    pub round: i32,
    pub ore: i32,
    /// Banked Steel and Coal are separate industrial resources.
    pub steel: i32,
    pub coal: i32,
    /// Banked crystal (the strategic resource for the deep tech).
    pub crystal: i32,
    /// Estimated next-turn income from the HQ and claimed resource tiles.
    pub resource_income: ResourceBundle,
    /// Accrued research points (generated by Tech Labs).
    pub research_points: i32,
    /// Number of completed technologies.
    pub researched_count: usize,
    /// Whether a technology is currently being researched.
...
impl FeatureInput {
    ...
    pub fn from_game(game: &Game, player: Player) -> Self {
        ...
        let research = &game.research[player.index()];
        let (power_produced, power_consumed) = game.power(player);
        FeatureInput {
            turn: game.turn,
            round: game.round,
            ore: game.ore[player.index()],
            steel: game.steel[player.index()],
            coal: game.coal[player.index()],
            crystal: game.crystal[player.index()],
            resource_income: game.resource_income(player),
            research_points: research.points,
            researched_count: research.researched.len(),
            researching: research.researching.is_some(),
            power_produced,
            power_consumed,
            own_units,
            own_buildings,
...
///         (each sector is MAP_SIZE/8 tiles wide)
...
/// [93]    research points / 600
/// [94]    researching flag (0/1)
/// [95]    researched count / 10
/// [96]    banked crystal / 200
/// [97]    own gunships / 8
...
/// [105]   own scouts / 8
/// [106]   own rocket troopers / 8
/// [107]   own sam launchers / 4
/// [108]   own crystal refineries / 2
/// [109]   own AA turrets / 4
/// [110]   observed enemy scout/rocket infantry (decayed) / 8
/// [111]   observed enemy sam launchers (decayed) / 4
/// [112]   banked steel / 2000
/// [113]   banked coal / 2000
/// [114]   estimated steel income / 1000
/// [115]   estimated coal income / 1000
/// [116]   estimated crystal income / 1000
/// [117]   low-power deficit / 500
/// [118]   own construction sites / 8
/// [119]   player-facing round / timeout
/// ```
pub fn extract_single(input: &FeatureInput) -> Vec<f32> {
    ...
    // The new roster: scouts, rocket troopers, SAM launchers, crystal
    // refineries and AA turrets (previously reserved slots).
    f[105] = clamp01(count_own(input, UnitType::Scout) as f32 / 8.0);
    f[106] = clamp01(count_own(input, UnitType::RocketTrooper) as f32 / 8.0);
    f[107] = clamp01(count_own(input, UnitType::SamLauncher) as f32 / 4.0);
    f[108] = clamp01(count_buildings(input, BuildingType::CrystalRefinery) as f32 / 2.0);
    f[109] = clamp01(count_buildings(input, BuildingType::AATurret) as f32 / 4.0);

    // Research state (schema v7 replaced the old upgrade one-hot).
    f[93] = clamp01(input.research_points as f32 / 600.0);
    f[94] = if input.researching { 1.0 } else { 0.0 };
    f[95] = clamp01(input.researched_count as f32 / 10.0);
    f[96] = clamp01(input.crystal as f32 / 200.0);
    f[112] = clamp01(input.steel as f32 / 2000.0);
    f[113] = clamp01(input.coal as f32 / 2000.0);
    ...
    for m in &input.fog.units {
        ...
        match m.utype {
            ...
            UnitType::Scout | UnitType::RocketTrooper => f[110] = clamp01(f[110] + w / 8.0),
            ...
            UnitType::SamLauncher => f[111] = clamp01(f[111] + w / 4.0),
    ...
    for m in &input.fog.buildings {
        ...
        let idx = match m.btype {
            BuildingType::Hq => 16,
            BuildingType::Refinery | BuildingType::PowerPlant | BuildingType::CrystalRefinery => 17,
            BuildingType::Factory => 18,
            BuildingType::Barracks => 19,
            BuildingType::TechLab | BuildingType::Airfield => 20,
            BuildingType::Turret | BuildingType::AATurret => 21,
    ...
    let (flip_x, flip_y) = (
        input.own_hq_tile.0 >= map_mid,
        input.own_hq_tile.1 >= map_mid,
    );
    for m in &input.fog.buildings {
        let sector_size = MAP_SIZE / 8;
        let mut sx = (m.tile.0 as usize) / sector_size;
        let mut sy = (m.tile.1 as usize) / sector_size;
        if flip_x {
            sx = 7 - sx;
        }
        if flip_y {
    ...
    // The server's resource income estimate is authoritative; the fallback
    // formula keeps this feature meaningful for old hand-built observations.
    let income = count_buildings(input, BuildingType::Refinery) as i32
        * REFINERY_BASE_YIELD_PER_TURN
        + HQ_INCOME_PER_TURN;
    f[101] = clamp01(input.resource_income.ore.max(income) as f32 / 1000.0);
    f[114] = clamp01(input.resource_income.steel as f32 / 1000.0);
    f[115] = clamp01(input.resource_income.coal as f32 / 1000.0);
    f[116] = clamp01(input.resource_income.crystal as f32 / 1000.0);

    // Operational pressure and construction context. These are own-state
    // facts, so they remain legal even when the enemy is fully hidden.
    f[117] = clamp01((input.power_consumed - input.power_produced).max(0) as f32 / 500.0);
    let construction_sites = input
        .own_buildings
        .iter()
        .filter(|building| {
        ...
        .count();
    f[118] = clamp01(construction_sites as f32 / 8.0);
    f[119] = clamp01(input.round as f32 / MATCH_TIMEOUT_TURNS.max(1) as f32);
...
mod tests {
    ...
    fn feature_dim_is_stable() {
        ...
        assert_eq!(f.len(), 240);
        assert_eq!(extract_single(&input).len(), 120);
    }
    ...
    fn history_stacks_previous_observations() {
        ...
        assert_eq!(no_hist.len(), 240);
        assert!(no_hist[..120].iter().all(|&v| v == 0.0));
        assert_eq!(&no_hist[120..], &single[..]);

        // One previous activation: it fills the first 120 slots verbatim.
        let hist = extract(&input, std::slice::from_ref(&single));
        assert_eq!(&hist[..120], &single[..]);
        assert_eq!(&hist[120..], &single[..]);

        // A longer history is trimmed to the oldest-first window.
        let two = vec![vec![1.0f32; 120], single.clone()];
        let trimmed = extract(&input, &two);
        assert_eq!(&trimmed[..120], &two[0][..]);
        assert_eq!(&trimmed[120..], &single[..]);
    }
